...
<table id="mytable" class="table table-striped m-4 border" style="width:100%">
    ...
    <tbody id="tbapp">
        ...
        WHERE id_apprentie NOT IN (
            SELECT apprenties.id_apprentie
            FROM apprenties
            INNER JOIN af ON af.id_apprentie = apprenties.id_apprentie
            INNER JOIN filiere ON af.id_filiere = filiere.id_filiere
        )
        GROUP BY apprenties.nom_apprentie;
        ";
                  
        $sql111 = "SELECT * FROM `af`,apprenties WHERE apprenties.id_apprentie=af.id_apprentie";
        
                   
        $res11 = mysqli_query($db, $sql11);
        $res111 = mysqli_query($db, $sql111);

        if ($res11 && mysqli_num_rows($res11) > 0) {
            while ($i = mysqli_fetch_array($res11)) {
                if ($i['photo_apprentie']=="") {
                    # code...
                    $i['photo_apprentie']='logoame.jpg';
                }
                echo "
                <tr>
                  <td class='text-danger'> <a href='#'><img src='assets/uploads/" . $i['photo_apprentie'] . "' class='alae' style='width: 50px;border-radius:50%;height: 50px;'></a></td>
                    <td class='text-danger'><a href='#'>" . $i['nom_apprentie'] . "</a></td>
                    <td class='text-danger'><a href='#'>" . $i['prenom_apprentie'] . "</a></td>
                    <td class='text-danger'><a href='#'>" . $i['genre_apprentie'] . "</a></td>
                    <td class='text-danger'><a href='#'>" . $i['telephone_apprentie'] . "</a></td>
                    <td class='text-danger'><a href='#'>" . $i['adresse_apprentie'] . "</a></td>
